...
import RelationshipsModal from './modals/NpcsModal';
...
const GameScreen: React.FC<GameScreenProps> = (props) => {
    const { 
        ...
        onUpdateLocation, onDeleteLocation, hasStoryStarted, onStartStory
    } = props;
    ...
    const renderActiveView = () => {
        ...
        switch (viewType) {
            ...
            case ActiveView.Management:
                ...
                />;
            case ActiveView.Relationships:
                return <RelationshipsModal
                    npcs={gameState.npcs}
    ...
    return (
        <div className={`game-screen ${isRightSidebarExpanded ? 'right-expanded' : 'right-collapsed'}`}>
            {isLeftSidebarExpanded && <div className="mobile-overlay" onClick={() => setIsLeftSidebarExpanded(false)}></div>}
            <aside className={`game-sidebar-left ${isLeftSidebarExpanded ? 'expanded' : 'collapsed'}`}>
                {/* Content is now always rendered, visibility is controlled by CSS */}
                <>
            ...
            <main className="game-main-content">
                {!hasStoryStarted ? (
                    <div className="start-story-container">
                        <h2 className="start-story-title">Chào mừng đến với {gameState.world.setting}!</h2>
                        <p className="start-story-p">Cuộc phiêu lưu của bạn sắp bắt đầu. Khi đã sẵn sàng, hãy nhấn nút bên dưới.</p>
                        <Button onClick={onStartStory} isLoading={isLoading} variant="primary" className="start-story-btn">
                            Bắt đầu nhập vai
                        </Button>
                    </div>
                ) : (
            ...
            <aside className={`game-sidebar-right ${isRightSidebarExpanded ? 'expanded' : ''}`}>
                ...
                <ToolbarIcon onClick={() => handleSetView(ActiveView.Lore)} label="Tri Thức" isActive={activeView?.type === ActiveView.Lore} isExpanded={isRightSidebarExpanded}>
                 ...
                 <ToolbarIcon onClick={() => handleSetView(ActiveView.Relationships)} label="Mối Quan Hệ" isActive={activeView?.type === ActiveView.Relationships} isExpanded={isRightSidebarExpanded}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m-7.14 2.72a9.094 9.094 0 0 1 3.741-.479 3 3 0 0 1-4.682-2.72M13.5 3a9 9 0 0 1 8.824 12.235 9 9 0 0 1-17.648 0A9 9 0 0 1 13.5 3Z" />
                    </svg>
                </ToolbarIcon>
