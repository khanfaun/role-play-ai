...
export async function fetchSkillDetailsFromAI(trait: InitialSkill, worldStyle: string, userApiKey: string): Promise<Skill> {
  const prompt = `
...
export async function generateSummary(storyText: string, userApiKey: string): Promise<string> {
...
export async function callGeminiAPI(prompt: string, userApiKey: string): Promise<{ story: string, choices: string[], tags: string }> {
  try {
    const response = await callBackend({
      model: "gemini-2.5-flash",
      contents: prompt,
...
export async function analyzeSourceMaterial(sourceTitle: string, sourceAuthor: string | undefined, userApiKey: string): Promise<SourceMaterialAnalysis | null> {
...
export async function generatePrimaryWorldDetails(prompt: string, userApiKey: string): Promise<AIWorldGenerationResponse | null> {
  const schema: any = {
    type: Type.OBJECT,
    properties: {
      world: {
...
export async function generateInitialAddons(prompt: string, userApiKey: string): Promise<{ initialAddons: InitialAddons } | null> {
